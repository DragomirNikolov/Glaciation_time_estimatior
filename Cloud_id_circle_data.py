...
cloud_mask = np.ma.masked_where(~(((Y - shape[2]/2 - 10*T)**2 + (X - centery - int(radius*1.5))**2 > radius**2) ^ (
    (Y - shape[2]/2 - 10*T)**2 + (X - centery+int(radius*1.5))**2 > radius2**2)), cloud_mask)
feature_varname = np.zeros(shape)
feature_varname[np.where(~((Y - shape[2]/2 - 10*T)**2 +
                         (X - centery - int(radius*1.5))**2 > radius**2))] = 1
feature_varname[np.where(~((Y - shape[2]/2 - 10*T)**2 +
                         (X - centery + int(radius*1.5))**2 > radius2**2))] = 2
...
for i in time_ind_cph:
    ...
    for var_name, var in cph_data.variables.items():
        if var_name == 'time':
            ...
            new_time[:] = nc.date2num(
                [date], units=time_units, calendar=calendar)
        elif var_name == "x":
            ...
            new_var[:] = ~(cloud_mask.mask[count, :, :])

            new_var = new_dataset.createVariable(
                "feature_number", var.dtype, ('time', 'lat', 'lon'))
            # Copy variable attributes
            new_var.setncatts({k: var.getncattr(k) for k in var.ncattrs()})
            # For time-dependent variables, use the ith timestep -(cloud_mask.mask[count,:,:]-1)*50
            new_var[:] = feature_varname[count, :, :]
    ...
    basetime = new_dataset.createVariable(
        'base_time', 'f8', ('time',), fill_value=np.nan)
    ...
    npix_feature = new_dataset.createVariable(
        'npix_feature', 'u8', ('object_num'))
