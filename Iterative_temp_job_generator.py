...
for i in range(len(min_temp_array)):
    ...
    temp_setup['root_path'] = work_dir + \
        f'/TEST/example_postprocessing/T-{round(abs(min_temp))}-{round(abs(max_temp))}'
    ...
    current_job_content[
        2] = f'#SBATCH -o /cluster/work/climate/dnikolo/Job_output/Output/pyflex_T-{round(abs(min_temp))}-{round(abs(max_temp))}.out\n'
    current_job_content[
        3] = f'#SBATCH -e /cluster/work/climate/dnikolo/Job_output/Error/pyflex_T-{round(abs(min_temp))}-{round(abs(max_temp))}.err\n'
