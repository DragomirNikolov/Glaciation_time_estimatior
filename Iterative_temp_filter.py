...
# ==================================================
def output_file_generation(comb_mask,y_limited_ind_bug,dates):
    date=dates[0]
    new_file_name = f"/wolke_scratch/dnikolo/TEST/example_preprocessing/CTT-it-38-0-{date.day:02d}-{date.month:02d}-{date.year}_{date.hour:02d}:{date.minute:02d}:{date.second:02d}.nc"
    ...
    for var_name, var in cph_data.variables.items():
        if var_name == 'time':
            ...
            new_var.setncatts({k: var.getncattr(k) for k in var.ncattrs()})  # Copy variable attributes
            new_var[:] = ~(comb_mask) # For time-dependent variables, use the ith timestep -(cloud_mask.mask[count,:,:]-1)*50
...
def filter_data(ctt,cph,min_temp,max_temp):
    ...
    ctt= np.ma.masked_where((ctt < 273.15 +min_temp) | (ctt>273.15+max_temp), ctt)
...
filter_mask=filter_data(ctt,cph,-38,0)


# ==================================================
# ==================================================
print(f"Generating new merged file")
output_file_generation(filter_mask,y_limited_ind_bug,dates)
print(f"New merged file generated and saved. Elapsed time {time.time()-start_time}")
